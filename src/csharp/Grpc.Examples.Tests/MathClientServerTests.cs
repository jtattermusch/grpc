            ...
            using (var call = client.DivMany())
            {
                var writeThread = new Thread(async () =>
                {
                    try
                    {
                        ...
                        await call.RequestStream.WriteAsync(new DivArgs { Dividend = 100, Divisor = 21 });
                        await call.RequestStream.WriteAsync(new DivArgs { Dividend = 7, Divisor = 2 });
                        await call.RequestStream.CompleteAsync();
                        Console.WriteLine("Completed the request stream.");
                    } catch(Exception e)
                    {
                       Console.WriteLine("write thread: " + e);
                    }
                });
                writeThread.Start();
                ...
                var readThread = new Thread(async () =>
                {
                    ...
                    {
                        Console.WriteLine("read thread: " + e);
                    }
                });
                readThread.Start();
