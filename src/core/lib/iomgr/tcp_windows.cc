  ...
  if (error == GRPC_ERROR_NONE) {
    if (info->wsa_error != 0 && !tcp->shutting_down) {
      ...
      if (info->bytes_transfered != 0 && !tcp->shutting_down) {
        ...
        if (static_cast<size_t>(info->bytes_transfered) != tcp->read_slices->length) {
          grpc_slice_buffer_trim_end(
            tcp->read_slices,
            tcp->read_slices->length - static_cast<size_t>(info->bytes_transfered),
...
static void win_read(grpc_endpoint* ep, grpc_slice_buffer* read_slices,
  ...
  if (tcp->read_slices->length < DEFAULT_TARGET_READ_SIZE / 2 &&
    ...
    grpc_slice_buffer_add(tcp->read_slices, GRPC_SLICE_MALLOC(DEFAULT_TARGET_READ_SIZE));
  }
  ...
  status =
      WSARecv(tcp->socket->socket, buffers, (DWORD)tcp->read_slices->count, &bytes_read, &flags, NULL, NULL);
  ...
  status = WSARecv(tcp->socket->socket, buffers, (DWORD)tcp->read_slices->count, &bytes_read, &flags,
